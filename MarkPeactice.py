...
def marking_sheet():
    ...
    def Add():
        if (len(roll.get()) != 0):
            College_Marksheet_Backend.insert(name.get(), roll.get(), father_name.get(), mother_name.get(),
                                             ...
                                             grand_tot.get(), percentage.get(), cgpa.get(), grade.get(), division.get(),
                                             result.get())
    ...
    Compute_button = Button(root, text='COMPUTE', font=('arial', 12, 'bold'), bg="#D2E603", width=10, command=Compute)
...
def search_result_marksheet(row):
    ...
    dt_fr1 = Frame(root, bd=4, relief=RIDGE, bg="Black")
